...
const Meetings: React.FC<MeetingsProps> = ({ user: propUser }) => {
  ...
  const renderActiveMeeting = () => (
    <div
      ...
      className="fixed inset-0 bg-[#0A0A0A] text-white flex flex-col z-[100] animate-fade-in font-sans"
    >
      {/* Top Bar (Zoom style) */}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 bg-black flex items-center justify-center p-4 relative">
            ...
            <div className="w-full h-full flex flex-col">
              ...
              <div className="flex-1 bg-[#1A1A1A] relative rounded overflow-hidden border border-white/10 mb-4">
                {!isVideoOff ? (
                  <canvas
                    ref={canvasRef}
                    className={`w-full h-full object-contain ${!isSharingScreen ? 'scale-x-[-1]' : ''}`}
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="w-32 h-32 bg-gray-500 rounded-full flex items-center justify-center text-4xl font-bold">
                      YO
                    </div>
                  </div>
                )}
              ...
              {participants.length > 0 && (
                <div className="h-32 flex gap-4 overflow-x-auto">
                  {participants.map((p, i) => (
                    <div
                      key={i}
                      className="aspect-video bg-[#1A1A1A] rounded border border-white/10 relative flex-shrink-0"
                    >
          ...
          {viewMode === 'grid' && (
            <div
              className={`grid gap-4 w-full max-w-7xl ${participants.length === 0 ? 'grid-cols-1' : 'grid-cols-2 md:grid-cols-3'} aspect-video`}
            >
              {/* Self Video */}
              <div className="bg-[#1A1A1A] relative rounded overflow-hidden border border-white/10 group">
                {!isVideoOff ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="w-24 h-24 bg-gray-500 rounded-full flex items-center justify-center text-3xl font-bold">
                      YO
                    </div>
                  </div>
                )}
                <div className="absolute bottom-2 left-2 bg-black/60 px-2 py-0.5 rounded text-[11px] font-medium">
              ...
              {participants.map((p, i) => (
                <div
                  key={i}
                  className={`bg-[#1A1A1A] relative rounded overflow-hidden border border-white/10 ${p.isSpeaking ? 'ring-2 ring-green-500' : ''}`}
                >
                  {p.isVideoOff ? (
                    <div className="w-full h-full flex items-center justify-center">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between p-2 hover:bg-gray-100 rounded">
                    ...
                    <div className="flex items-center gap-1 text-gray-500">
                      ...
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M15 8v8H5l-5-6 5-6h10zm1 0h2v8h-2V8zm4 0h2v8h-2V8z" />
                      </svg>
                    </div>
                  </div>
                  {participants.map((p, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 text-gray-500">
                        <button
                          onClick={() => setBots(bots.filter(pt => pt.id !== p.id))}
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="border-t border-gray-200 p-2">
                    <div className="flex items-center gap-2 bg-gray-100 rounded-full px-4 py-2">
                      ...
                      />
                      <button
                        onClick={() => setChatMessage(chatMessage + ' 🎤 (Voice)')}
      ...
      <div className="h-20 bg-[#1A1A1A] flex items-center justify-between px-4 shrink-0 overflow-x-auto custom-scrollbar gap-4">
          ...
          <div className="relative">
            {showAudioMenu && (
              <div className="fixed bottom-24 left-4 bg-[#242424] rounded-lg shadow-xl border border-white/10 p-2 w-64 z-[110]">
                <div className="text-xs font-bold text-gray-400 px-2 py-1 uppercase">
                  Select Microphone
                </div>
                ...
                <button
                  ...
                  className="w-full text-left px-2 py-2 hover:bg-white/10 rounded text-sm"
                >
                  Audio Settings...
                </button>
              </div>
            ...
            <div className="flex items-center rounded hover:bg-[#2A2A2A] transition-colors">
              <button
                ...
                className="flex flex-col items-center gap-1 min-w-[50px] p-2 text-white/90"
              >
                <div className="relative">
                  {isMuted ? (
                    <svg className="w-6 h-6 text-red-500" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.02.17c0-.06.02-.11.02-.17V5c0-1.66-1.34-3-3-3S9 3.66 9 5v.18l5.98 5.99zM4.27 3L3 4.27l6.01 6.01V11c0 1.66 1.33 3 2.99 3 .22 0 .44-.03.65-.08l2.97 2.97c-.85.35-1.76.57-2.71.5V21h2v-3.28c3.28-.48 5.69-3.18 5.69-6.6v-1.73l1.89 1.89 1.27-1.27L4.27 3z" />
                    </svg>
                  ) : (
              ...
              <button
                ...
                className="h-full px-1 hover:bg-[#3A3A3A] rounded-r flex items-center justify-center text-gray-500 hover:text-white"
        ...
        <div className="flex items-center gap-2">
          {[
              ...
              menu: (
                <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-[#242424] rounded-lg shadow-xl border border-white/10 p-2 w-48 z-[110]">
                  ...
                  <button
                    ...
                    className="w-full text-left px-2 py-2 hover:bg-white/10 rounded text-sm flex items-center justify-between"
                  >
            ...
            },
            {
              ...
              color: isSharingScreen ? 'text-green-500' : '',
              icon: (props: unknown) => (
                <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
                  <path d="M20 18c1.1 0 1.99-.9 1.99-2L22 6c0-1.11-.9-2-2-2H4c-1.11 0-2 .89-2 2v10c0 1.1.89 2 2 2H0v2h24v-2h-4zM4 16V6h16v10.01L4 16zm9-6.88V13h-2V9.12l-2.83 2.83-1.41-1.41L12 5.29l5.24 5.25-1.41 1.41L13 9.12z" />
                </svg>
              ),
            },
            {
              ...
              color: isRecording ? 'text-red-500' : '',
              icon: (props: unknown) => (
                <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
            ...
            <div key={i} className="relative">
              ...
              >
                ...
                {btn.count && (
                  <span className="absolute top-2 ml-4 bg-red-500 text-white text-[9px] px-1 rounded-full">
                    {btn.count}
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-4 shrink-0">
          <button
            onClick={endMeeting}
            className="bg-[#E01E5A] hover:bg-[#C21648] text-white px-4 py-1.5 rounded-lg text-sm font-bold transition-colors"
          >
            End
          </button>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase tracking-widest">
                  <span className="bg-[#050819] px-4 text-white/40">Or Join</span>
                </div>
              </div>
            ...
            <div className="flex-1 relative aspect-video bg-black/50 flex items-center justify-center group">
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4">
                <button
                  ...
                  className={`p-3 rounded-full backdrop-blur-md border transition-all ${isMuted ? 'bg-red-500/20 border-red-500/50 text-red-500' : 'bg-white/10 border-white/10 hover:bg-white/20'}`}
                >
                  {isMuted ? (
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.02.17c0-.06.02-.11.02-.17V5c0-1.66-1.34-3-3-3S9 3.66 9 5v.18l5.98 5.99zM4.27 3L3 4.27l6.01 6.01V11c0 1.66 1.33 3 2.99 3 .22 0 .44-.03.65-.08l2.97 2.97c-.85.35-1.76.57-2.71.5V21h2v-3.28c3.28-.48 5.69-3.18 5.69-6.6v-1.73l1.89 1.89 1.27-1.27L4.27 3z" />
                ...
                <button
                  ...
                  className={`p-3 rounded-full backdrop-blur-md border transition-all ${isVideoOff ? 'bg-red-500/20 border-red-500/50 text-red-500' : 'bg-white/10 border-white/10 hover:bg-white/20'}`}
                >
                  {isVideoOff ? (
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M21 6.5l-4 4V7c0-.55-.45-1-1-1H9.82L21 17.18V6.5zM3.27 2L2 3.27 4.73 6H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.21 0 .39-.08.54-.18L19.73 21 21 19.73 3.27 2z" />
