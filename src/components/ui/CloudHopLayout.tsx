...
import { Home, MessageSquare, Music, Gamepad2, Globe, Tv, Video, ChevronLeft, ChevronRight, Settings, User } from "lucide-react";
...
const NAV_ITEMS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "hophub", label: "HopHub", icon: <MessageSquare className="w-5 h-5" /> },
  ...
  { id: "hopmeetings", label: "HopMeetings", icon: <Video className="w-5 h-5" /> },
];
...
export default function CloudHopLayout({ activeTab, onTabChange, children }: CloudHopLayoutProps) {
    ...
    <div className="flex h-screen w-full overflow-hidden relative">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-3 py-4 border-b border-white/10">
          <img src={logoSplash} alt="CloudHop" className="w-10 h-10 rounded-lg object-contain flex-shrink-0" />
          {!collapsed && (
            <div className="min-w-0">
              <h1 className="text-sm font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-magenta-400 truncate">
                CloudHop
              </h1>
              <p className="text-[10px] text-muted-foreground truncate">Hop In, Cloud On</p>
            </div>
          )}
        </div>
        ...
        <div className="px-3 py-3 border-t border-white/10 flex items-center gap-3">
          ...
          {!collapsed && (
            <div className="min-w-0">
              <p className="text-xs font-semibold text-foreground truncate">CloudHopper</p>
