...
const XPBar: React.FC<XPBarProps> = ({ xp, level, nextLevelXP, compact = false }) => {
  ...
  return (
    <div className={`w-full ${compact ? 'space-y-1' : 'space-y-3'}`}>
      {!compact && (
        <div className="flex justify-between items-end">
          <div className="flex items-baseline gap-2">
            <span className="text-xs font-black uppercase tracking-widest text-[#53C8FF]">
              Level
            </span>
      ...
      <div
        className={`w-full ${compact ? 'h-1' : 'h-2'} bg-white/5 rounded-full overflow-hidden border border-white/5`}
      >
        <div
