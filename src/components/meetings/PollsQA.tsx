...
export default function PollsQA({ onClose }: PollsQAProps) {
  ...
  const [questions, setQuestions] = useState<Question[]>([
    {
      id: '1',
      text: 'When will the next sprint start?',
      author: 'Alex Chen',
      upvotes: 3,
      answered: false,
    },
    {
      id: '2',
      text: 'Can we share the recording?',
      author: 'Jordan Dev',
      upvotes: 1,
      answered: false,
    },
  ...
  const createPoll = () => {
    ...
    setPolls([
      ...polls,
      {
        ...
        options: pollOptions
          .filter((o) => o.trim())
          .map((o, i) => ({ id: String(i), text: o, votes: 0 })),
        isActive: true,
      },
    ]);
    setPollQuestion('');
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          {!showCreatePoll ? (
            <button
              ...
              className="w-full text-xs px-3 py-2 rounded bg-white/5 border border-dashed border-cyan-400/30 text-cyan-300 hover:bg-white/10 transition-all flex items-center justify-center gap-1"
            >
            ...
            <div className="p-3 rounded-lg bg-white/5 border border-cyan-400/20 space-y-2">
              <input
                ...
                className="w-full text-xs px-2 py-1.5 bg-white/5 border border-cyan-400/30 rounded text-foreground placeholder-muted-foreground outline-none"
              />
              {pollOptions.map((opt, i) => (
                <div key={i} className="flex gap-1">
                  <input
                    ...
                    className="flex-1 text-xs px-2 py-1.5 bg-white/5 border border-cyan-400/30 rounded text-foreground placeholder-muted-foreground outline-none"
                  />
              ...
              <div className="flex gap-2">
                <button
                  onClick={createPoll}
                  className="flex-1 text-xs px-2 py-1.5 rounded bg-cyan-500/20 border border-cyan-400/50 text-cyan-300"
                >
                  Create
                </button>
                <button
                  onClick={() => setShowCreatePoll(false)}
                  className="text-xs px-2 py-1.5 rounded text-muted-foreground"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          <div className="flex gap-2 mb-2">
            ...
            <button
              ...
              className="text-xs px-2 py-1.5 rounded bg-cyan-500/20 border border-cyan-400/50 text-cyan-300"
            >
              Ask
            </button>
          </div>
            ...
            .map((q) => (
              ...
              >
                <button
                  ...
                  className="flex flex-col items-center gap-0.5 text-muted-foreground hover:text-cyan-400 transition-all"
                >
