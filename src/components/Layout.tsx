...
const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate, user, onLogout }) => {
  ...
  return (
    <div className={`flex h-screen overflow-hidden italic selection:bg-[#53C8FF]/30 ${bgClass}`}>
      {/* Sidebar */}
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-24'} ${sidebarClass} flex flex-col transition-all duration-500 border-r relative z-20 shadow-[20px_0_60px_rgba(0,0,0,0.5)]`}>
        <button 
          className="p-4 flex items-center gap-3 h-16 border-b cursor-pointer hover:bg-white/5 transition-all overflow-hidden w-full text-left" 
        ...
        <header className={`h-16 shrink-0 backdrop-blur-2xl flex items-center justify-between px-6 border-b z-10 ${headerClass}`}>
          <div className="flex items-center gap-4">
            <button 
              ...
              className={`p-2 rounded-lg transition-all ${isLightMode ? 'hover:bg-gray-100 text-slate-600' : 'hover:bg-white/5 text-white/80 hover:text-white'}`} 
            ...
            <div className="relative">
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-[#10233A] border border-[#1E3A5F] rounded-xl shadow-2xl p-2 z-50 animate-fade-in overflow-hidden">
                  <button 
                    onClick={() => {onNavigate(View.PROFILE); setShowProfileMenu(false);}} 
                  ...
                  <button 
                    onClick={() => {onNavigate(View.SETTINGS); setShowProfileMenu(false);}} 
                    className="w-full flex items-center gap-3 px-3 py-2 hover:bg-white/5 rounded-lg text-sm font-semibold uppercase tracking-wide transition-all italic"
                  >
                    Setup
                  </button>
