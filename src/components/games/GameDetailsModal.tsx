          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-start gap-4">
              <div className="flex-1">
                <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">{currentGame.title}</h1>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                    ...
                    {currentGame.status === 'available' ? 'Available' : currentGame.status === 'coming-soon' ? 'Coming Soon' : 'Unavailable'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {currentGame.tags.map(tag => (
                <span key={tag} className="px-3 py-1 bg-white/10 rounded-lg text-sm text-gray-300 border border-white/20">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {currentGame.playTime && (
              <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
                ...
                <div className="text-white font-semibold">{formatPlayTime(currentGame.playTime)}</div>
