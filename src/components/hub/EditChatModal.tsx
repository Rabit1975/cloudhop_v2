...
export const EditChatModal: React.FC<EditChatModalProps> = ({ chat, onSave, onClose }) => {
  const [name, setName] = useState(chat.title || '');
  const [description, setDescription] = useState(chat.description || '');
  // const [avatar, setAvatar] = useState(chat.avatar || ''); // Currently not using custom avatars for chats in schema, but good to have
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Edit Chat">
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          >
            {loading && (
              <div className="w-4 h-4 border-2 border-[#0A0F1F]/30 border-t-[#0A0F1F] rounded-full animate-spin" />
            )}
            Save Changes
          </button>
        </div>
