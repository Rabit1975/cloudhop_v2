...
import { ForcedUpdatesPanel } from './ForcedUpdatesPanel';
...
const Settings: React.FC<SettingsProps> = ({ userId }) => {
  ...
  const [showForcedUpdates, setShowForcedUpdates] = useState(false);
  const [repoOwner, setRepoOwner] = useState('Rabit1975');
  const [repoName, setRepoName] = useState('CloudHop_v2');
    ...
    try {
      const newStream = await navigator.mediaDevices.getUserMedia({
        audio: {
          ...
          noiseSuppression:
            settings.suppressNoise !== 'Auto' && settings.suppressNoise !== undefined,
        },
      });
    ...
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row bg-[#0E1430] border border-white/5 rounded-[32px] overflow-hidden shadow-2xl min-h-[700px] animate-fade-in italic">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-12 bg-[#0E1430] relative">
        <div className="mb-12 border-b border-white/5 pb-8">
          ...
          <p className="text-white/30 text-base font-medium italic">
            Configure your CloudHop experience.
          </p>
        </div>
        ...
        {tab === 'General' && (
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Appearance">
              <SettingItem
                title="Color Mode"
                desc="Change the background color of the CloudHop desktop app."
              >
                <select
            ...
            <SettingGroup title="System">
              <SettingItem
                title="Start CloudHop when I start Windows"
                desc="Launch automatically on startup."
              >
                <Toggle
                  active={settings.startOnBoot}
                  onToggle={() => updateSetting('startOnBoot', !settings.startOnBoot)}
                />
              </SettingItem>
              <SettingItem
                title="When closed, minimize window to notification area"
                desc="Keep CloudHop running in the background."
              >
                <Toggle
                  active={settings.minimizeToTray}
                  onToggle={() => updateSetting('minimizeToTray', !settings.minimizeToTray)}
                />
              </SettingItem>
              <SettingItem
                title="Use dual monitors"
                desc="Show participants and content on separate screens."
              >
                <Toggle
                  active={settings.dualMonitors}
                  onToggle={() => updateSetting('dualMonitors', !settings.dualMonitors)}
                />
              </SettingItem>
              <SettingItem title="Enter full screen automatically when starting or joining a meeting">
                <Toggle
                  active={settings.autoFullScreen}
                  onToggle={() => updateSetting('autoFullScreen', !settings.autoFullScreen)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
        ...
        {tab === 'Video & effects' && (
          <div className="space-y-12 animate-fade-in">
            <div className="space-y-6">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-white/30 italic">
                Live Lens Preview
              </h4>
              <div className="relative aspect-video w-full max-w-xl bg-black rounded-[40px] overflow-hidden border-4 border-white/5 shadow-2xl group">
                <video
                  ...
                  playsInline
                  muted
                  className={`w-full h-full object-cover transition-transform duration-700 ${settings.mirrorVideo ? 'scaleX(-1)' : ''}`}
                />
                {!isCameraOn && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#080C22] space-y-4">
                    ...
                    <button
                      ...
                      className="px-8 py-3 bg-[#53C8FF] text-[#0A0F1F] rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl italic hover:scale-105 transition-all"
                    >
                      Enable Optics
                    </button>
                  </div>
            ...
            <SettingGroup title="Camera">
              ...
              <SettingItem title="Original Ratio" desc="Maintain aspect ratio.">
                <Toggle
                  active={settings.originalRatio}
                  onToggle={() => updateSetting('originalRatio', !settings.originalRatio)}
                />
              </SettingItem>
              <SettingItem title="HD Video" desc="Enable 1080p streaming.">
                <Toggle
                  active={settings.hdVideo}
                  onToggle={() => updateSetting('hdVideo', !settings.hdVideo)}
                />
              </SettingItem>
              <SettingItem title="Mirror Video" desc="Flip your video preview.">
                <Toggle
                  active={settings.mirrorVideo}
                  onToggle={() => updateSetting('mirrorVideo', !settings.mirrorVideo)}
                />
              </SettingItem>
            </SettingGroup>
            <SettingGroup title="My Video">
              ...
              <SettingItem title="Always display participant names">
                <Toggle
                  active={settings.alwaysShowNames}
                  onToggle={() => updateSetting('alwaysShowNames', !settings.alwaysShowNames)}
                />
              </SettingItem>
              <SettingItem title="Stop my video when joining">
                <Toggle
                  active={settings.stopVideoOnJoin}
                  onToggle={() => updateSetting('stopVideoOnJoin', !settings.stopVideoOnJoin)}
                />
              </SettingItem>
              <SettingItem title="Always show video preview dialog when joining">
                <Toggle
                  active={settings.showPreviewOnJoin}
                  onToggle={() => updateSetting('showPreviewOnJoin', !settings.showPreviewOnJoin)}
                />
              </SettingItem>
              <SettingItem title="Hide non-video participants">
                <Toggle
                  active={settings.hideNonVideo}
                  onToggle={() => updateSetting('hideNonVideo', !settings.hideNonVideo)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
          ...
          <div className="space-y-12 animate-fade-in">
            ...
            <SettingGroup title="Microphone">
              ...
              <div className="space-y-2">
                ...
                <div className="h-2 w-full max-w-md bg-white/5 rounded-full overflow-hidden flex gap-1 px-0.5 items-center">
                  {Array.from({ length: 20 }).map((_, i) => (
                    <div
                      key={i}
                      className={`h-1 flex-1 rounded-full transition-all duration-100 ${inputLevel > i * 5 ? 'bg-[#53C8FF]' : 'bg-white/5'}`}
                    ></div>
                  ))}
                </div>
              </div>
              <SettingItem title="Automatically adjust microphone volume">
                <Toggle
                  active={settings.autoMicVolume}
                  onToggle={() => updateSetting('autoMicVolume', !settings.autoMicVolume)}
                />
              </SettingItem>
            </SettingGroup>
            ...
            <SettingGroup title="Audio Profile">
              ...
              <SettingItem title="Show in-meeting option to 'Enable Original Sound'">
                <Toggle
                  active={settings.originalSound}
                  onToggle={() => updateSetting('originalSound', !settings.originalSound)}
                />
              </SettingItem>
              <SettingItem title="Echo Cancellation">
                <Toggle
                  active={settings.echoCancellation}
                  onToggle={() => updateSetting('echoCancellation', !settings.echoCancellation)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
          ...
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Notifications">
              <SettingItem
                ...
                desc="Display a popup when you receive a message."
              >
                <Toggle
                  active={settings.showNotificationBanner}
                  onToggle={() =>
                    updateSetting('showNotificationBanner', !settings.showNotificationBanner)
                  }
                />
              </SettingItem>
              <SettingItem title="Bounce dock icon" desc="Animate the dock icon on new activity.">
                <Toggle
                  active={settings.bounceDockIcon}
                  onToggle={() => updateSetting('bounceDockIcon', !settings.bounceDockIcon)}
                />
              </SettingItem>
            </SettingGroup>
            <SettingGroup title="Sounds">
              <SettingItem title="Play sound for incoming message">
                <Toggle
                  active={settings.playSoundMessage}
                  onToggle={() => updateSetting('playSoundMessage', !settings.playSoundMessage)}
                />
              </SettingItem>
              <SettingItem title="Play sound when someone joins or leaves">
                <Toggle
                  active={settings.playSoundJoin}
                  onToggle={() => updateSetting('playSoundJoin', !settings.playSoundJoin)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
          ...
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="General">
              <SettingItem title="Copy invite link when starting a meeting">
                <Toggle
                  active={settings.copyInviteLinkOnStart}
                  onToggle={() =>
                    updateSetting('copyInviteLinkOnStart', !settings.copyInviteLinkOnStart)
                  }
                />
              </SettingItem>
              <SettingItem title="Always show meeting controls">
                <Toggle
                  active={settings.alwaysShowControls}
                  onToggle={() => updateSetting('alwaysShowControls', !settings.alwaysShowControls)}
                />
              </SettingItem>
              <SettingItem title="Ask me to confirm when I leave a meeting">
                <Toggle
                  active={settings.confirmLeave}
                  onToggle={() => updateSetting('confirmLeave', !settings.confirmLeave)}
                />
              </SettingItem>
              <SettingItem title="Show meeting timer">
                <Toggle
                  active={settings.showMeetingTimer}
                  onToggle={() => updateSetting('showMeetingTimer', !settings.showMeetingTimer)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
          ...
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Window Size">
              ...
              <SettingItem title="Scale to fit shared content">
                <Toggle
                  active={settings.scaleToFit}
                  onToggle={() => updateSetting('scaleToFit', !settings.scaleToFit)}
                />
              </SettingItem>
              <SettingItem title="See shared content in side-by-side mode">
                <Toggle
                  active={settings.sideBySide}
                  onToggle={() => updateSetting('sideBySide', !settings.sideBySide)}
                />
              </SettingItem>
            </SettingGroup>
            <SettingGroup title="When I Share">
              <SettingItem title="Silence system notifications when sharing desktop">
                <Toggle
                  active={settings.silenceNotifications}
                  onToggle={() =>
                    updateSetting('silenceNotifications', !settings.silenceNotifications)
                  }
                />
              </SettingItem>
        ...
        {tab === 'Recording' && (
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Local Recording">
              <SettingItem
                title="Store recordings at:"
                desc={settings.recordingPath || 'C:\\Users\\CloudHop\\Documents\\Zoom'}
              >
                <button className="px-4 py-2 bg-[#1A2348] rounded text-xs text-[#53C8FF]">
                  Change
                </button>
              </SettingItem>
              <SettingItem title="Choose a location for recorded files when the meeting ends">
                <Toggle
                  active={settings.chooseLocationOnEnd}
                  onToggle={() =>
                    updateSetting('chooseLocationOnEnd', !settings.chooseLocationOnEnd)
                  }
                />
              </SettingItem>
              <SettingItem title="Record separate audio file for each participant">
                <Toggle
                  active={settings.separateAudio}
                  onToggle={() => updateSetting('separateAudio', !settings.separateAudio)}
                />
              </SettingItem>
              <SettingItem title="Optimize for 3rd party video editor">
                <Toggle
                  active={settings.optimizeForEditor}
                  onToggle={() => updateSetting('optimizeForEditor', !settings.optimizeForEditor)}
                />
              </SettingItem>
              <SettingItem title="Add a timestamp to the recording">
                <Toggle
                  active={settings.addTimestamp}
                  onToggle={() => updateSetting('addTimestamp', !settings.addTimestamp)}
                />
              </SettingItem>
              <SettingItem title="Record video during screen sharing">
                <Toggle
                  active={settings.recordVideoDuringShare}
                  onToggle={() =>
                    updateSetting('recordVideoDuringShare', !settings.recordVideoDuringShare)
                  }
                />
              </SettingItem>
            </SettingGroup>
          ...
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="General">
              <SettingItem title="Link Preview" desc="Show preview for links shared in chat.">
                <Toggle
                  active={settings.linkPreview}
                  onToggle={() => updateSetting('linkPreview', !settings.linkPreview)}
                />
              </SettingItem>
              <SettingItem title="File Transfer" desc="Allow sending files in chat.">
                <Toggle
                  active={settings.fileTransfer}
                  onToggle={() => updateSetting('fileTransfer', !settings.fileTransfer)}
                />
              </SettingItem>
              <SettingItem title="Play Animated GIFs" desc="Automatically play GIFs.">
                <Toggle
                  active={settings.animatedGifs}
                  onToggle={() => updateSetting('animatedGifs', !settings.animatedGifs)}
                />
              </SettingItem>
              <SettingItem
                title="Code Snippet Mode"
                desc="Enable syntax highlighting for code blocks."
              >
                <Toggle
                  active={settings.codeSnippet}
                  onToggle={() => updateSetting('codeSnippet', !settings.codeSnippet)}
                />
              </SettingItem>
            </SettingGroup>
          </div>
        ...
        {tab === 'Plans' && (
          <div className="space-y-12 animate-fade-in">
            <div className="text-center mb-8">
              <h3 className="text-2xl font-black uppercase italic tracking-tighter text-[#53C8FF]">
                Choose Your Plan
              </h3>
              <p className="text-white/60 mt-2">
                Flexible plans for casual users, creators, and teams.
              </p>
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="bg-[#050819] border border-white/5 rounded-2xl p-6 flex flex-col">
                ...
                <div className="text-3xl font-bold my-4">
                  $0<span className="text-sm text-white/40 font-normal"> / forever</span>
                </div>
                ...
                <button className="w-full py-3 bg-white/10 rounded-xl font-bold text-xs uppercase tracking-wider">
                  Current Plan
                </button>
              </div>
              ...
              <div className="bg-[#050819] border border-white/5 rounded-2xl p-6 flex flex-col">
                ...
                <div className="text-3xl font-bold my-4">
                  $5.99<span className="text-sm text-white/40 font-normal"> / mo</span>
                </div>
                <ul className="space-y-2 text-xs text-white/70 flex-1 mb-6">
                ...
                <button className="w-full py-3 bg-[#53C8FF] text-[#0A0F1F] rounded-xl font-bold text-xs uppercase tracking-wider">
                  Upgrade
                </button>
              </div>
              ...
              <div className="bg-[#050819] border-2 border-[#53C8FF] rounded-2xl p-6 flex flex-col relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-[#53C8FF] text-[#0A0F1F] text-[9px] font-bold px-2 py-1 rounded-bl-lg uppercase">
                  Popular
                </div>
                <h3 className="text-lg font-bold text-[#53C8FF]">Pro Plan</h3>
                <div className="text-3xl font-bold my-4">
                  $14.99<span className="text-sm text-white/40 font-normal"> / mo</span>
                </div>
                <ul className="space-y-2 text-xs text-white/70 flex-1 mb-6">
                ...
                <button className="w-full py-3 bg-[#53C8FF] text-[#0A0F1F] rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-[#53C8FF]/20">
                  Get Pro
                </button>
              </div>
              ...
              <div className="bg-[#050819] border border-white/5 rounded-2xl p-6 flex flex-col">
                ...
                <div className="text-3xl font-bold my-4">
                  $8.99<span className="text-sm text-white/40 font-normal"> / user / mo</span>
                </div>
                <ul className="space-y-2 text-xs text-white/70 flex-1 mb-6">
                ...
                <button className="w-full py-3 bg-white/10 hover:bg-white/20 rounded-xl font-bold text-xs uppercase tracking-wider">
                  Contact Sales
                </button>
              </div>
            </div>
          ...
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Captions">
              <SettingItem title="Closed Caption Font Size">
                <label htmlFor="caption-font-size" className="block text-white/60 text-sm mb-2">
              ...
              <SettingItem title="Always show captions">
                <Toggle
                  active={settings.alwaysShowCaptions}
                  onToggle={() => updateSetting('alwaysShowCaptions', !settings.alwaysShowCaptions)}
                />
              </SettingItem>
            </SettingGroup>
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-[#050819] border border-white/5 rounded-2xl mb-8">
              <div className="relative group mb-6">
                <img
                  ...
                  className="w-32 h-32 rounded-full object-cover border-4 border-[#53C8FF] shadow-[0_0_40px_rgba(83,200,255,0.3)]"
                />
              ...
              <h3 className="text-2xl font-black text-white italic tracking-tight">
                {profile.display_name || 'Cloud Hopper'}
              </h3>
        ...
        {tab === 'GitHub' && (
          <div className="space-y-12 animate-fade-in">
            <SettingGroup title="Repository Management">
              <SettingItem
                title="Show Forced Updates"
                desc="Check for force-pushed updates on all branches in your repository."
              >
                <button
                  onClick={() => setShowForcedUpdates(true)}
                  className="bg-[#53C8FF] text-white px-6 py-3 rounded-xl hover:bg-[#4AB8FF] transition-colors font-black uppercase italic text-xs"
                >
                  Check Forced Updates
                </button>
              </SettingItem>
              <SettingItem title="Repository" desc="Configure which GitHub repository to monitor.">
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={repoOwner}
                  ...
                  <input
                    ...
                    className="bg-[#050819] border border-white/10 rounded-xl px-4 py-3 text-xs font-black uppercase italic w-48 text-[#53C8FF]"
                    id="repoName"
                  />
                </div>
              </SettingItem>
      ...
      {showForcedUpdates && (
        <ForcedUpdatesPanel
          repoOwner={repoOwner}
          repoName={repoName}
          onClose={() => setShowForcedUpdates(false)}
        />
      )}
    </div>
  );
...
const SettingGroup: React.FC<{ title: string; children: React.ReactNode }> = ({
  ...
  <div className="space-y-8">
    <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-[#53C8FF] pb-4 border-b border-white/5 italic">
      {title}
    </h3>
...
const SettingItem: React.FC<{ title: string; desc?: string; children: React.ReactNode }> = ({
  title,
  desc,
  children,
}) => (
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 group">
    <div className="max-w-xl text-left">
      <h4 className="text-lg font-black text-white italic group-hover:text-[#53C8FF] transition-colors uppercase tracking-tight">
        {title}
      </h4>
      {desc && (
        <p className="text-sm text-white/30 mt-1 italic font-medium leading-relaxed">{desc}</p>
      )}
    </div>
...
const Toggle: React.FC<{ active?: boolean; onToggle?: () => void }> = ({
  active = false,
  onToggle,
}) => {
  return (
    ...
    >
      <span
        className={`w-4.5 h-4.5 bg-white rounded-full transition-all shadow-xl ${active ? 'translate-x-7' : 'translate-x-0'}`}
      ></span>
    </button>
  );
};
