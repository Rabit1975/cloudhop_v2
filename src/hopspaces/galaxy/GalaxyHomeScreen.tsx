      ...
      <GalaxyBackground mood={galaxyMood} className="absolute inset-0">
          ...
          >
              ...
              {spaces.map(
                (s, i) =>
                  i > 0 && (
                    <line
                      key={`line-${i}`}
                      x1={`${spaces[i - 1].galaxy_position.x}%`}
                      y1={`${spaces[i - 1].galaxy_position.y}%`}
                      x2={`${s.galaxy_position.x}%`}
                      y2={`${s.galaxy_position.y}%`}
                      stroke="white"
                      strokeWidth="1"
                    />
                  )
            ...
            {spaces.map(space => (
              ...
              >
                {/* Planet Visual */}
                <div
                  className={`relative w-full h-full rounded-full overflow-hidden border-2 border-white/10 group-hover:border-[#53C8FF] shadow-[0_0_30px_rgba(0,0,0,0.5)] transition-all duration-500 ${
                    space.mood === 'intense'
                      ? 'shadow-red-500/20'
                      : space.mood === 'dreamy'
                ...
                <motion.div
                  ...
                  whileHover={{ y: 0 }}
                >
                  <span className="text-sm font-black uppercase tracking-widest whitespace-nowrap bg-black/50 px-3 py-1 rounded-full backdrop-blur-md border border-white/10">
                    {space.name}
                  </span>
                  <span className="text-[9px] text-[#53C8FF] font-bold uppercase tracking-wider mt-1">
                    {space.type}
                  </span>
                </motion.div>
              </motion.div>
        ...
        <div className="absolute top-8 left-8 z-50 pointer-events-none">
          <div className="pointer-events-auto">
            ...
            <p className="text-white/40 text-xs font-black uppercase tracking-[0.4em] ml-1 mt-2">
              Galaxy View • {galaxyMood} Sector
            </p>
          </div>
        </div>
          ...
          <div className="pointer-events-auto bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-2xl max-w-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-[#53C8FF]">
                Leonardo • Galactic Architect
              </span>
            </div>
            <p className="text-xs text-white/80 italic leading-relaxed">
              "The galaxy is calm today. Your creative flow suggests we should build a new{' '}
              <strong>Music Space</strong>. Shall we?"
            </p>
          </div>
        </div>
      </GalaxyBackground>
