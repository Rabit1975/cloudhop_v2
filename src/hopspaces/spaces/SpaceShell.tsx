    ...
    <div className="relative w-full h-full overflow-hidden bg-[#050819]">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-4 flex justify-between items-center bg-gradient-to-b from-black/50 to-transparent pointer-events-none">
        <div className="pointer-events-auto flex items-center gap-4">
          ...
          <span className="px-2 py-0.5 rounded-full border border-white/20 text-[10px] text-white/60 uppercase tracking-widest">
            {space.type}
          </span>
        </div>
      </div>
      ...
      <div className="absolute inset-0 z-0">
        <img
          src={space.background_url || space.cover_image_url}
          className="w-full h-full object-cover opacity-30 blur-sm"
        />
      </div>

      {/* Content */}
      <div className="relative z-10 w-full h-full pt-16">{children}</div>
    </div>
  );
};
