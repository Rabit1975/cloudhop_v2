...
export function stepConstellation(
  ...
  const evolved = maybeGrowConstellation({ ...current, nodes, edges, resonance, turbulence, gravity, growth });
...
function getResonanceForMood(mood: string): number {
  switch (mood) {
    ...
    case "frost": return 0.6;
    case "drift": default: return 1.0;
  }
}

function getTurbulenceForMood(mood: string): number {
  switch (mood) {
    case "bloom": return 0.2;
    case "surge": return 0.6;
    case "storm": return 0.9;
    case "frost": return 0.1;
    case "drift": default: return 0.3;
  }
}

function getGravityForMood(mood: string): number {
  switch (mood) {
    case "bloom": return 0.4;
    case "surge": return 0.8;
    case "storm": return 0.6;
    case "frost": return 0.2;
    case "drift": default: return 0.5;
  }
}

function getGrowthForMood(mood: string): number {
  switch (mood) {
    case "bloom": return 0.9;
    case "surge": return 0.8;
