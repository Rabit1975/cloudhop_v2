...
export function useConstellationEvolution(sectionClass: string) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      constellation.clusters.forEach(cluster => {
        const targetGravity = constellation.season === 'surge' ? 0.8 : constellation.season === 'frost' ? 0.2 : 0.5;
        ...
        if (nodesInCluster.length > 0) {
          const centerX = nodesInCluster.reduce((sum, node) => sum + node.x, 0) / nodesInCluster.length;
          const centerY = nodesInCluster.reduce((sum, node) => sum + node.y, 0) / nodesInCluster.length;
      ...
      if (constellation.season === 'bloom' || constellation.season === 'surge') {
        if (Math.random() < 0.02) { // 2% chance per frame
          const clusterId = Array.from(constellation.clusters.keys())[Math.floor(Math.random() * constellation.clusters.size)];
  ...
  const addEdge = (from: string, to: string, strength: number = 0.5, archetype: string = 'connection') => {
