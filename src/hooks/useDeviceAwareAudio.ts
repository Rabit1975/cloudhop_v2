...
function getDeviceProfile(): DeviceProfile {
  ...
  const isHeadphones = 
    ...
    (navigator as any).mediaDevices?.enumerateDevices?.().then((devices: any[]) => {
...
export function createDeviceAwareStereoPanner(ctx: AudioContext, deviceProfile: DeviceProfile): StereoPannerNode {
