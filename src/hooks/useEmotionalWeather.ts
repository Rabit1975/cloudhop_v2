...
function playWeather({ type, volume = 0.1 }: { type: keyof typeof weatherProfiles; volume?: number }) {
  try {
...
function animateWeather(filter: BiquadFilterNode, profile: typeof weatherProfiles[keyof typeof weatherProfiles]) {
  let t = 0;
...
function getArchetypeFromTags(tags: string[]): keyof typeof archetypeEQ {
  ...
  return (primaryTag in archetypeEQ) 
    ? primaryTag as keyof typeof archetypeEQ 
    : 'default';
}
