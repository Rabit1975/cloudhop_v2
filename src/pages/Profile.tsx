...
import { Camera, Edit3, Save, MapPin, Link2, Calendar, Star, MessageSquare, Gamepad2, Music, Clock, Trophy, Heart } from "lucide-react";
...
export default function Profile() {
  ...
  const [bio, setBio] = useState("Just a cosmic hopper exploring the cloud ☁️✨");
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative h-48 rounded-2xl overflow-hidden mb-16 glass-panel border-cyan-400/20">
            ...
            <div className="absolute -bottom-12 left-8 z-10">
              <div className="relative">
                ...
                <div className={cn("absolute top-1 right-1 w-4 h-4 rounded-full border-2 border-background", status === "online" ? "bg-green-500" : status === "away" ? "bg-yellow-500" : "bg-gray-500")} />
              </div>
            </div>

            {/* Edit button */}
            <button onClick={() => setEditing(!editing)} className="absolute top-4 right-4 px-4 py-2 rounded-lg glass-panel border-cyan-400/30 hover:border-cyan-400/60 text-foreground text-sm font-medium flex items-center gap-2 transition-all z-10">
          ...
          <div className="mb-8 pl-2">
            <div className="flex items-center gap-4 mb-2">
              {editing ? (
                <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="text-3xl font-black bg-transparent border-b-2 border-cyan-400/50 text-foreground outline-none" />
              ) : (
                <h1 className="text-3xl font-black text-foreground">{displayName}</h1>
              )}
              <span className="px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-400/30 text-cyan-300 text-xs font-bold">PRO</span>
            </div>
            {editing ? (
              <input value={statusMessage} onChange={(e) => setStatusMessage(e.target.value)} className="text-sm bg-transparent border-b border-cyan-400/30 text-cyan-300 outline-none mb-3 w-64" />
            ) : (
              <p className="text-sm text-cyan-300 mb-3">💬 {statusMessage}</p>
            )}
            {editing ? (
              <textarea value={bio} onChange={(e) => setBio(e.target.value)} className="text-sm bg-white/5 border border-cyan-400/30 text-muted-foreground rounded-lg p-3 outline-none w-full resize-none" rows={2} />
            ) : (
            ...
            {editing && (
              <div className="mt-4 flex gap-3">
                <select value={status} onChange={(e) => setStatus(e.target.value)} className="bg-white/5 border border-cyan-400/30 rounded-lg px-3 py-2 text-sm text-foreground outline-none">
