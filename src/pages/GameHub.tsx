          ...
          {featuredGame && (
            <div className="mb-12">
              <button
                ...
                className="w-full group glass-panel rounded-2xl overflow-hidden border-cyan-400/40 hover:border-cyan-400/80 transition-all relative hover:shadow-2xl hover:shadow-cyan-500/30"
              >
                ...
                <div className="p-8 relative z-10 text-left">
                  <h2 className="text-4xl font-black mb-2 text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-magenta-300">
                    {featuredGame.name}
                  </h2>
                  <div className="flex items-center gap-4 mb-4">
                    <span className="px-4 py-2 rounded-full bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 font-semibold text-sm">
                  ...
                  <p className="text-muted-foreground text-lg mb-6 max-w-2xl">
                    Jump into {featuredGame.name} and experience an incredible
                    gaming adventure!
                  </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {trendingGames.map((game) => (
                <button
                  ...
                  className="group relative rounded-lg overflow-hidden border border-orange-400/30 hover:border-orange-400/80 transition-all h-32 hover:shadow-lg hover:shadow-orange-500/30"
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <p className="text-xs font-bold text-foreground text-center px-2 line-clamp-2">
                      {game.name}
                    </p>
                  </div>
                  <div className="absolute top-2 right-2 bg-gradient-to-r from-orange-500 to-red-500 px-2 py-1 rounded text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                    {game.category}
                  </div>
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className={cn(
                  ...
                  !selectedCategory
                    ? 'bg-gradient-to-r from-cyan-500 to-magenta-500 text-white border-transparent shadow-lg shadow-cyan-500/50'
                    : 'bg-white/5 border-cyan-400/30 text-muted-foreground hover:border-cyan-400/60 hover:text-foreground'
                )}
              >
                All Games
              </button>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedCategory === cat
                      ? 'bg-gradient-to-r from-cyan-500 to-magenta-500 text-white border-transparent shadow-lg shadow-cyan-500/50'
                      : 'bg-white/5 border-cyan-400/20 text-muted-foreground hover:border-cyan-400/60 hover:text-foreground'
                  )}
                >
                  {cat}
                </button>
          ...
          {filteredGames.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4 mb-12">
              {filteredGames.map((game) => (
                <button
                  key={game.id}
                  onClick={() => setSelectedGame(game)}
                  className="group glass-panel rounded-xl overflow-hidden border-cyan-400/30 hover:border-cyan-400/80 transition-all text-left relative hover:shadow-2xl hover:shadow-cyan-500/30 hover:scale-105"
                >
                  ...
                  <div className="relative h-48 bg-gradient-to-br from-cyan-900/30 via-purple-900/20 to-magenta-900/20 overflow-hidden flex items-center justify-center border-b border-cyan-400/20 group-hover:border-cyan-400/40">
                    <div className="text-6xl opacity-50 group-hover:opacity-70 transition-opacity transform group-hover:scale-110 duration-300">
                      🎮
                    </div>
                  </div>
                  <div className="p-5 relative z-10">
                    <h3 className="font-black text-foreground text-base mb-2 group-hover:text-cyan-300 transition-colors line-clamp-2">
                      {game.name}
                    </h3>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-magenta-400 font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="text-center py-16">
              ...
              <button
                ...
                className="mt-4 px-6 py-2 rounded-lg bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 hover:bg-cyan-500/30 transition-all"
              >
                Clear Search
              </button>
            </div>
