...
export default function Chat() {
  ...
  const [dmMessages, setDmMessages] = useState<Message[]>([
    ...
    {
      id: '2',
      content: 'Pretty good! Just finished the Twitch integration',
      sender: 'user',
      username: 'You',
      avatar: 'U',
  ...
  const handleAddAttachment = (
    ...
    setAttachments([
      ...attachments,
      {
        ...
        name: `file_${Date.now()}.${type === 'image' ? 'jpg' : type === 'video' ? 'mp4' : type === 'audio' ? 'mp3' : 'pdf'}`,
        type,
        url: '#',
  ...
  const handleSendGift = (giftType: string) => {
    ...
    setGiftsReceived((prev) => ({
      ...prev,
      [selectedDM.userId]: (prev[selectedDM.userId] || 0) + 1,
    }));
          ...
          <div className="flex-1 glass-panel rounded-xl overflow-hidden flex flex-col border-cyan-400/30">
                ...
                {giftsReceived[selectedDM.userId] > 0 && (
                  <div className="flex items-center gap-1 px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-400/30">
                    ...
                    <span className="text-xs font-semibold text-cyan-300">
                      {giftsReceived[selectedDM.userId]}
                    </span>
                  </div>
                )}
                <div className="relative">
                  ...
                  {showGiftMenu && (
                    <div className="absolute right-0 top-full mt-1 bg-card border border-cyan-400/50 rounded-lg p-2 flex flex-col gap-1 z-50">
                      <button
                        onClick={() => handleSendGift('birthday gift')}
                        className="px-3 py-1.5 text-sm text-left hover:bg-cyan-500/20 rounded transition-all"
                      >
                        🎂 Birthday
                      </button>
                      <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
              {dmMessages.map((message) => (
                ...
                >
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 bg-gradient-to-br from-cyan-500 to-blue-600 text-white">
                    {message.avatar}
                  </div>
                  <div
                    className={cn(
              ...
              <div className="border-t border-cyan-400/20 p-3 bg-cyan-500/5">
                <div className="flex flex-wrap gap-2">
                  {attachments.map((att) => (
                    <div
                      key={att.id}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-cyan-500/20 border border-cyan-400/30 text-sm"
                    >
                      {att.type === 'image' && (
                      ...
                      <span className="text-xs text-cyan-300 truncate max-w-32">
                        {att.name}
                      </span>
                      <button
                        onClick={() => handleRemoveAttachment(att.id)}
                        className="ml-auto p-0.5 hover:bg-white/10 rounded text-muted-foreground hover:text-foreground"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
              ...
              <div className="group relative">
                ...
                <div className="absolute bottom-full mb-2 left-0 bg-card border border-cyan-400/50 rounded-lg p-2 hidden group-hover:flex flex-col gap-1 z-50 whitespace-nowrap">
                  <button
                    ...
                    className="px-3 py-1.5 text-sm text-left hover:bg-cyan-500/20 rounded transition-all flex items-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-sm text-left hover:bg-cyan-500/20 rounded transition-all flex items-center gap-2"
                  >
                    <MusicIcon className="w-4 h-4" /> Audio
                  </button>
                </div>
              </div>
              <input
                ...
                className={cn(
                  'flex-1 rounded-lg px-4 py-2 bg-white/5 border border-cyan-400/30 text-foreground placeholder-muted-foreground outline-none',
                  'focus:border-cyan-400/60 focus:bg-white/10 transition-all'
                )}
              />
              <button
                onClick={handleSendDM}
                disabled={!dmInput.trim() && attachments.length === 0}
                className={cn(
                  'px-4 py-2 rounded-lg font-medium transition-all',
                  dmInput.trim() || attachments.length > 0
    ...
    <CloudHopLayout activeTab={activeTab} onTabChange={setActiveTab}>
          ...
          <div className="px-4 py-4 border-b border-cyan-400/20 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 hover:bg-cyan-400/10 rounded-lg transition-all text-cyan-400 hover:text-cyan-300"
              >
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full pl-10 pr-3 py-2 bg-white/5 border border-cyan-400/30 rounded-lg text-sm text-foreground placeholder-muted-foreground outline-none focus:border-cyan-400/60 focus:bg-white/10 transition-all"
              />
            </div>
            <button
              onClick={() => setShowCreateGroupModal(true)}
              className="w-full px-3 py-2 rounded-lg bg-magenta-500/20 border border-magenta-400/50 text-magenta-300 hover:bg-magenta-500/30 transition-all text-sm font-medium flex items-center justify-center gap-2 mt-2"
            >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-cyan-400/20">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 hover:bg-cyan-400/10 rounded-lg transition-all text-muted-foreground hover:text-cyan-400 relative"
              >
                <Play className="w-5 h-5" />
                {showCallMenu && (
                  <div className="absolute top-full mt-1 right-0 bg-card border border-cyan-400/50 rounded-lg p-2 flex flex-col gap-1 z-50 whitespace-nowrap">
                    <button className="px-3 py-1.5 text-sm text-left hover:bg-cyan-500/20 rounded transition-all flex items-center gap-2">
                      <Play className="w-3 h-3" /> Start Voice Call
                    </button>
                    <button className="px-3 py-1.5 text-sm text-left hover:bg-cyan-500/20 rounded transition-all flex items-center gap-2">
                      <Users className="w-3 h-3" /> Start Video Call
                    </button>
                  </div>
                )}
              </button>
        ...
        <div className="w-72 glass-panel rounded-xl overflow-hidden flex flex-col border-magenta-400/30">
          <div className="flex border-b border-magenta-400/20">
            <button
              ...
              className={cn(
                'flex-1 px-4 py-3 font-semibold text-sm border-b-2 transition-all',
                activeTabView === 'messages'
                  ? 'border-magenta-400 text-magenta-300 bg-magenta-500/10'
            ...
            <button
              onClick={() => setActiveTabView('requests')}
              className={cn(
                'flex-1 px-4 py-3 font-semibold text-sm border-b-2 transition-all',
                activeTabView === 'requests'
                  ? 'border-magenta-400 text-magenta-300 bg-magenta-500/10'
                  : 'border-transparent text-muted-foreground hover:text-foreground'
              )}
            >
                  ...
                  {directMessages.slice(0, 5).map((dm) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div className="relative flex-shrink-0">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-magenta-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs">
                            {dm.avatar}
                          </div>
                          <div
                            className={cn(
              ...
              <div className="space-y-3 p-3">
                {friendRequests.length > 0 ? (
                  friendRequests.map((request) => (
                    <div
                      key={request.id}
                      className="glass-panel rounded-lg p-3 border-magenta-400/20"
                    >
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 px-3 py-1.5 rounded-lg bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 hover:bg-cyan-500/30 transition-all text-xs font-medium flex items-center justify-center gap-1"
                        >
                          <UserCheck className="w-3 h-3" /> Accept
                        </button>
                        <button
                          onClick={() => handleDeclineFriendRequest(request.id)}
                          className="flex-1 px-3 py-1.5 rounded-lg bg-red-500/20 border border-red-400/50 text-red-300 hover:bg-red-500/30 transition-all text-xs font-medium"
                        >
                          Decline
                        </button>
                      </div>
                  ...
                  <div className="p-6 text-center">
                    <UserPlus className="w-8 h-8 text-muted-foreground mx-auto mb-2 opacity-30" />
                    <p className="text-xs text-muted-foreground">
                      No friend requests
                    </p>
                  </div>
      ...
      {showCreateChannelModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md glass-panel rounded-xl border-cyan-400/30 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white/5 border border-cyan-400/30 rounded-lg text-foreground placeholder-muted-foreground outline-none focus:border-cyan-400/60 focus:bg-white/10"
                />
              </div>
              <div>
                <label className="text-sm font-semibold text-foreground mb-2 block">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <Hash className="w-4 h-4 inline mr-2" /> Public
                  </button>
                  <button
            ...
            <div className="flex gap-2 mt-6">
              <button
                onClick={() => setShowCreateChannelModal(false)}
                className="flex-1 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/10 transition-all"
              >
                Cancel
              </button>
              <button
