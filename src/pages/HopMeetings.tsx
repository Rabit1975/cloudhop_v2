...
export default function HopMeetings() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    ...
    },
    {
      id: '2',
      sender: 'You',
      message: 'Yes, perfect sound quality!',
      timestamp: new Date(Date.now() - 90000),
    },
  ]);

  const [meetingInfo] = useState({
    meetingId: '829 4476 9876',
    meetingCode: 'CloudHop Meeting',
    startTime: new Date(),
  });
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-12 text-center">
              ...
              <p className="text-muted-foreground text-lg">
                Crystal-clear video conferencing for teams
              </p>
            </div>
            <div className="glass-panel rounded-2xl border-cyan-400/30 p-8 mb-8">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-6 py-4 rounded-lg bg-gradient-to-r from-cyan-500 to-cyan-400 text-black font-bold text-lg hover:opacity-90 transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-cyan-500/40"
                >
                  <Phone className="w-6 h-6" /> Start Meeting
                </button>
              </div>
              <p className="text-xs text-muted-foreground text-center mt-4">
                You can invite others by sharing the meeting ID
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-screen bg-gradient-to-b from-purple-900/20 via-transparent to-black p-6 flex items-center justify-center">
          <div className="glass-panel rounded-2xl border-cyan-400/30 p-12 max-w-md w-full text-center">
            ...
            <button
              ...
              className="w-full px-6 py-3 rounded-lg bg-gradient-to-r from-cyan-500 to-cyan-400 text-black font-bold hover:opacity-90 transition-all"
            >
              Start New Meeting
            </button>
          </div>
    ...
    <div className="h-full w-full overflow-hidden flex flex-col bg-black">
      ...
      <div className="glass-panel flex items-center justify-between px-6 py-3 border-b border-cyan-400/20 relative z-20">
        <div className="flex items-center gap-4">
          ...
          {isRecording && (
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-red-600/20 border border-red-400/50">
        ...
        <button
          ...
          className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold transition-all flex items-center gap-2"
        >
      ...
      <div className="flex-1 overflow-hidden flex gap-4">
            ...
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 p-2 auto-rows-fr">
                {participants.map((participant) => (
                  <div
                    key={participant.id}
                    className="relative group bg-gradient-to-br from-cyan-900/20 to-magenta-900/20 rounded-lg overflow-hidden border border-cyan-400/20 flex items-center justify-center"
                  >
              ...
              <div className="flex items-center justify-center h-full">
                <div className="relative w-full max-w-3xl aspect-video bg-gradient-to-br from-cyan-900/20 to-magenta-900/20 rounded-lg border border-cyan-400/30 flex items-center justify-center overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 glass-panel rounded-lg px-4 py-3 border-cyan-400/30">
                    <p className="text-foreground font-bold">
                      {participants[1].name} is presenting
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Speaking • Screen shared
                    </p>
                  </div>
                  <div className="absolute bottom-4 right-4 flex gap-2">
                    {participants.slice(0, 3).map((p) => (
                      <div
                        key={p.id}
                        className="w-24 h-16 rounded-lg bg-gradient-to-br from-cyan-600 to-magenta-600 border border-cyan-400/30 flex items-center justify-center text-white text-xs font-bold overflow-hidden"
                      >
                        {p.cameraOn ? '📹' : p.avatar}
                      </div>
                    ))}
          ...
          <div className="glass-panel border-t border-cyan-400/20 px-4 py-3 flex items-center justify-center gap-2 flex-wrap">
            <button
              ...
              className={cn(
                ...
                isMuted
                  ? 'bg-red-600/20 border border-red-400 text-red-400 hover:bg-red-600/30'
            ...
            <button
              ...
              className={cn(
                'p-3 rounded-full transition-all',
                isScreenSharing
                  ? 'bg-magenta-500/20 border border-magenta-400 text-magenta-300'
            ...
            <button
              ...
              className={cn(
                'p-3 rounded-full transition-all',
                isRecording
                  ? 'bg-red-600/20 border border-red-400 text-red-400 animate-pulse'
                  : 'bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/10'
              )}
            >
            ...
            <button
              ...
              className="p-3 rounded-full bg-white/5 border border-white/10 text-muted-foreground hover:bg-white/10 transition-all"
              title="Settings"
            >
        ...
        {showParticipants && (
          <div className="w-72 glass-panel overflow-hidden border-l border-cyan-400/20 flex flex-col">
            <div className="px-4 py-3 border-b border-cyan-400/20 bg-cyan-500/10">
            ...
            <div className="flex-1 overflow-y-auto space-y-1 p-2">
              {participants.map((participant) => (
                <div
                  key={participant.id}
                  className="px-3 py-2 rounded-lg hover:bg-white/5 transition-all"
                >
                  <div className="flex items-center gap-2 mb-1">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-magenta-500 flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
                      {participant.avatar}
                    </div>
                  ...
                  <div className="flex items-center gap-1 ml-10 text-xs">
                    {participant.isMuted ? (
        ...
        {showChat && (
          <div className="w-72 glass-panel overflow-hidden border-l border-magenta-400/20 flex flex-col">
            <div className="px-4 py-3 border-b border-magenta-400/20 bg-magenta-500/10">
              <p className="font-bold text-foreground text-sm">Meeting Chat</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-black/80 backdrop-blur-sm">
          <div className="flex items-center justify-between px-6 py-3 glass-panel border-b border-cyan-400/20">
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg bg-red-600/20 border border-red-400 text-red-300 font-semibold text-sm hover:bg-red-600/30 transition-all"
            >
              Close
            </button>
          </div>
          ...
          <WaitingRoom
            ...
            onAdmit={(id) =>
              setWaitingParticipants(
                waitingParticipants.filter((p) => p.id !== id)
              )
            }
            onDeny={(id) =>
              setWaitingParticipants(
